...
export default function Editor() {
  ...
  const [features, setFeatures] = useState([{ title: "", desc: "" }]);
  const { id } = useParams();
  ...
  if (loading)
    return (
      <div className="min-h-screen flex items-center justify-center bg-black text-white text-lg font-medium">
        Loading...
      </div>
    );
  ...
  return (
    <div className="flex min-h-screen bg-black text-white overflow-x-hidden">
      ...
      {/* LEFT PANEL */}
      <aside
        className={`w-full md:w-[360px] p-4 sm:p-5 md:p-6 border-r border-gray-800 bg-[#0f0f10]/80 backdrop-blur fixed md:relative z-40 h-screen md:h-auto overflow-y-auto transition-transform ${sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
            ...
            const isValidUrl =
              ...
              (() => {
                try {
                  ...
                  const hasImageHint = /(jpg|jpeg|png|gif|webp|svg)/i.test(
                    url.search,
                  );
